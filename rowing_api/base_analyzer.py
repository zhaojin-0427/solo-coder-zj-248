...
class BaseAnalyzer:
    def __init__(self, store: BaseDataStore, strategy: Optional[CoachStrategyConfig] = None):
        self.store = store
    ...
    def _get_sync_rate_weights(self) -> Tuple[float, float, float, float]:
        if self.strategy:
            return 0.35, 0.25, 0.25, 0.15
        return 0.4, 0.3, 0.3, 0.0

    def _get_stroke_rate_target(self, seat_avg_rates: Dict[int, float]) -> float:
        if self.strategy:
            return (self.strategy.stroke_rate_target_min + self.strategy.stroke_rate_target_max) / 2
        return np.mean(list(seat_avg_rates.values()))

    def _get_stroke_rate_suggestion_target(self, target_center: float) -> str:
        if self.strategy:
            return f"全队目标桨频区间: {self.strategy.stroke_rate_target_min:.0f}-{self.strategy.stroke_rate_target_max:.0f}桨/分钟"
        return f"全队目标桨频: {target_center:.1f}桨/分钟"

    def _get_angle_suggestion_extra(self) -> str:
        if self.strategy:
            return f"入水角度容忍区间: ±{self.strategy.entry_angle_tolerance:.1f}度"
        return ""

    def _get_force_description_extra(self) -> str:
        if self.strategy:
            return f" (权重: {self.strategy.force_balance_weight:.1f})"
        return ""

    def _get_rhythm_description_extra(self) -> str:
        if self.strategy:
            return f" (阈值: {self.strategy.rhythm_stability_threshold:.3f})"
        return ""

    def _get_force_weight(self) -> float:
        return self.strategy.force_balance_weight if self.strategy else 1.0

    def _get_rhythm_threshold(self) -> float:
        return self.thresholds["rhythm_cv_threshold"]
    ...
    def calculate_sync_rate(self) -> Optional[SyncRateResult]:
        ...
        w1, w2, w3, w4 = self._get_sync_rate_weights()
        if self.strategy and w4 > 0:
            rhythm_factor = 1 - min(1, stroke_rate_sync / self._get_rhythm_threshold())
            overall_sync = (
                stroke_rate_sync_norm * w1 + 
                angle_sync_norm * w2 + 
                force_sync_norm * w3 +
                rhythm_factor * w4
            )
    ...
    def _on_anomaly_detected(self, anomaly: AnomalyRecord) -> None:
        pass
    ...
    def _detect_stroke_rate_mismatch(self, by_seat: Dict[int, List[AlignedDataPoint]]) -> List[AnomalyRecord]:
        ...
        target_center = self._get_stroke_rate_target(seat_avg_rates)
    ...
    def _generate_stroke_rate_suggestion(self, seat_rates: Dict[int, float], 
        ...
        suggestions.append(self._get_stroke_rate_suggestion_target(target_center))
    ...
    def _generate_angle_suggestion(self, seat_angles: Dict[int, float],
        ...
        extra = self._get_angle_suggestion_extra()
        if extra:
    ...
    def _detect_force_imbalance(self, by_seat: Dict[int, List[AlignedDataPoint]]) -> List[AnomalyRecord]:
        ...
        if problem_seats:
            ...
            weight_info = self._get_force_description_extra()
    ...
    def _detect_rhythm_disorder(self, by_seat: Dict[int, List[AlignedDataPoint]]) -> List[AnomalyRecord]:
        ...
        if problem_seats:
            ...
            threshold_info = self._get_rhythm_description_extra()
    ...
    def calculate_stability_ranking(self, start_time: Optional[int] = None,
        ...
        rankings = []
        force_weight = self._get_force_weight()
        rhythm_threshold = self._get_rhythm_threshold()
